...
import { getGuestBookings } from '../services/firestoreService';
import { Booking, BookingStatus } from '../types';
import Spinner from '../components/ui/Spinner';
import Header from '../components/layout/Header';
import BookingCard from '../components/common/BookingCard';

type TripCategory = 'upcoming' | 'ongoing' | 'past';
...
const MyTripsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        const guestBookings = await getGuestBookings(currentUser.uid);
  ...
  const getFilteredBookings = () => {
    ...
    switch (activeTab) {
        ...
        case 'past':
            return bookings.filter(b => b.checkOut < now || b.bookingStatus === BookingStatus.COMPLETED || b.bookingStatus === BookingStatus.CANCELLED_BY_GUEST);
        default:
            return [];
    }
  };
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <main className="container mx-auto p-4 md:p-8">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-6">My Trips</h1>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredBookings.map(booking => (
              <BookingCard key={booking.bookingId} booking={booking} userRole="guest" />
            ))}
          </div>
        ) : (
          <div className="text-center py-12">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">No {activeTab} trips</h3>
            <p className="text-gray-500 dark:text-gray-400 mt-2">You don't have any trips in this category.</p>
          </div>
        )}
      </main>
    </div>
