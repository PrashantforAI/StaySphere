  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden flex flex-col">
      <div className="relative">
        ...
        <span className={`absolute top-2 right-2 text-xs font-medium px-2.5 py-0.5 rounded-full capitalize ${getStatusChipColor(property.status)}`}>
          {property.status.replace('_', ' ')}
        </span>
      </div>
      <div className="p-4 flex-grow flex flex-col">
        <h3 className="font-bold text-lg text-gray-900 dark:text-white truncate">{property.title || 'Untitled Listing'}</h3>
        <p className="text-gray-600 dark:text-gray-400 text-sm mb-3">{property.location.city || 'Location not set'}</p>
        ...
        <div className="mt-auto border-t border-gray-200 dark:border-gray-700 pt-3">
          <div className="flex items-center justify-between gap-2 text-sm">
             ...
             <div className="flex items-center gap-2">
                <Link to={editUrl} className="font-medium text-gray-600 dark:text-gray-300 hover:underline">
                  {property.status === PropertyStatus.IN_PROGRESS ? 'Continue' : 'Edit'}
                </Link>
