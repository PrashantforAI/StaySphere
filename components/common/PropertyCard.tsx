      ...
      <div className="p-4">
        <div className="flex justify-between items-start">
            ...
            <div className="flex items-center space-x-1 flex-shrink-0 ml-2">
                <StarIcon />
                <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">{property.rating.toFixed(1)}</span>
                <span className="text-sm text-gray-500 dark:text-gray-400">({property.reviewCount})</span>
            </div>
        </div>
        <p className="text-gray-600 dark:text-gray-400 text-sm mb-3">{property.location}</p>
        <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">
          ₹{property.price.toLocaleString('en-IN')} <span className="text-sm font-normal text-gray-500 dark:text-gray-400">/ night</span>
        </p>
      </div>
    </div>
  );
